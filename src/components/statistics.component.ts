...
import { Manufacturer, ActiveIngredient, Preparation } from '../models';
...
export class StatisticsComponent {
  ...
  topPreparations = computed(() => {
    ...
    this.dataService.diaryEntries().forEach(entry => {
  ...
  effectStats = computed(() => {
    const stats = new Map<string, { effectName: string, emoji: string, counts: Map<string, number> }>();
    this.dataService.diaryEntries().forEach(entry => {
      if (!entry.preparationId) return;
      entry.effects.forEach(effect => {
         if (!stats.has(effect.id)) {
            stats.set(effect.id, { effectName: effect.description, emoji: effect.emoji, counts: new Map() });
         }
